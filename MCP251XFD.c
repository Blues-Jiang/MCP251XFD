...
eERRORRESULT Init_MCP251XFD(MCP251XFD *pComp, const MCP251XFD_Config *pConf)
{
    ...
    if ((pComp->DriverConfig & MCP251XFD_DRIVER_SAFE_RESET) == 0) {
        Error = pComp->fnSPI_Init(pComp->InterfaceDevice, pComp->SPI_ChipSelect,
                                  pComp->SPIClockSpeed);      // Initialize the SPI interface only in case of no safe reset (the interface will be initialized in the reset at a safe speed)
    ...
    uint8_t CheckVal = ((uint8_t)(Config) & MCP251XFD_SFR_OSC8_CHECKFLAGS) |
                       MCP251XFD_SFR_OSC8_OSCRDY;    // Check if PLL Locked (if enabled), OSC clock is running and stable, and SCLKDIV is synchronized (if divided by 2)
    ...
    Error = MCP251XFD_SetGPIOPinsOutputLevel(pComp, pComp->GPIOsOutLevel,
                                             MCP251XFD_GPIO0_Mask | MCP251XFD_GPIO1_Mask); // Set GPIO pins output level before change to mode GPIO. This is to get directly the good output level when (if) pins will be in output mode
...
eERRORRESULT MCP251XFD_ReadData(MCP251XFD *pComp, uint16_t address, uint8_t* data, uint16_t size)
{
    ...
    const bool InRAM  = (address >= MCP251XFD_RAM_ADDR) &&
                        (address < (MCP251XFD_RAM_ADDR + MCP251XFD_RAM_SIZE)); // True if address is in the RAM region ; False if address is in Controller SFR or CAN SFR region
    ...
    while (size > 0) {
        ...
        *pBuf = Addr & 0xFF;                                                     // Set next byte of SPI command
        ...
        if ((pComp->DriverConfig & MCP251XFD_DRIVER_CLEAR_BUFFER_BEFORE_READ) > 0) {
            const size_t BuffUsedSize = ByteCount + (UseCRC ? 2 + 1 - 2 :
                                                     2);      // Here 2 for Command + 1 for Length - 2 for CRC (CRC on the SPI send is not checked by the controller thus will be set to 0 too), else just 2 for command
...
eERRORRESULT MCP251XFD_WriteData(MCP251XFD *pComp, uint16_t address, const uint8_t* data, uint16_t size)
{
    ...
    while (size > 0) {
        ...
        *pBuf = Addr & 0xFF;                                                    // Set next byte of SPI command
        ...
        //--- Now send the data through SPI interface ---
        const size_t ByteToWriteCount = ByteCount + (UseSafe ? (2 + 2) : (UseCRC ? (2 + 1 + 2) :
                                                                          2)); // In case of use CRC for write here are 2 bytes for Command + 1 for Length for not safe + 2 for CRC, else just 2 for command
...
eERRORRESULT MCP251XFD_CalculateBitTimeConfiguration(const uint32_t fsysclk, const uint32_t desiredNominalBitrate, const uint32_t desiredDataBitrate, MCP251XFD_BitTimeConfig *pConf)
{
    ...
    uint32_t BRP =
        MCP251XFD_NBRP_MAX;                                                      // Select the worst BRP value. Here all value from max to min will be tested to get the best tuple of NBRP and DBRP, identical TQ in both phases prevents quantization errors during bit rate switching
    while (--BRP >= MCP251XFD_NBRP_MIN) {
        ...
        if (desiredDataBitrate != MCP251XFD_NO_CANFD) {
            ...
            if (ErrorDTQ == 0) { ErrorDTQ = 1; }   // Calculate DTQ error
        ...
        if (desiredDataBitrate != MCP251XFD_NO_CANFD) {
            if (DTQbits < MCP251XFD_DTQBIT_MAX) {
                ErrorNTQ = (fsysclk - (desiredNominalBitrate * NTQbits * BRP));
                if (ErrorNTQ == 0) { ErrorNTQ = 1; }    // Calculate NTQ error
                ErrorDTQ = ((desiredDataBitrate * (DTQbits + 1) * BRP) - fsysclk);
                if (ErrorDTQ == 0) { ErrorDTQ = 1; }    // Calculate DTQ error with DTQbits+1
                ErrorTQ = (ErrorNTQ * ErrorDTQ);
                if (ErrorTQ <= MinErrorBR)                                                                           // If better error then
            ...
            if ((NTQbits < MCP251XFD_NTQBIT_MAX) && (DTQbits < MCP251XFD_DTQBIT_MAX)) {
                ...
                if (ErrorNTQ == 0) { ErrorNTQ = 1; } // Calculate NTQ error with NTQbits+1
                ErrorDTQ = ((desiredDataBitrate * (DTQbits + 1) * BRP) - fsysclk);
                if (ErrorDTQ == 0) { ErrorDTQ = 1; } // Calculate DTQ error with DTQbits+1
                ErrorTQ = (ErrorNTQ * ErrorDTQ);
                if (ErrorTQ <= MinErrorBR)                                                                           // If better error then
    ...
    if (desiredDataBitrate != MCP251XFD_NO_CANFD) {
        ...
        const uint32_t SSP = BestBRP *
                             DTSEG1;                            // In order to set the SSP to 80%, SSP = TDCO + TDCV (Equation 3-10 of MCP25XXFD Family Reference Manual). SSP is set to DBRP * (DPRSEG + DPHSEG1) = DBRP * DTSEG1
        uint32_t TDCO = SSP;
...
eERRORRESULT MCP251XFD_CalculateBitrateStatistics(const uint32_t fsysclk, MCP251XFD_BitTimeConfig *pConf, bool can20only)
{
    ...
    const uint32_t NPRSEG  = (pConf->NTSEG1 + 1) - (pConf->NTSEG2 +
                                                    1);             // Here PHSEG2 (NTSEG2) should be equal to PHSEG1 so NPRSEG = NTSEG1 - NTSEG2 (Figure 3-2 of MCP25XXFD Family Reference Manual)
    pConf->Stats->MaxBusLength = (uint32_t)(((NTQ * NPRSEG) - (2 * MCP251XFD_tTXDtRXD_MAX)) / (2 *
                                            MCP251XFD_tBUS_CONV)); // Formula is (2x(tTXD�RXD + (5*BusLen))/NTQ = NPRSEG => BusLen = ((NTQ*NPRESG)-(2*tTXD))/(2*5) in meter (Equation 3-9 of MCP25XXFD Family Reference Manual)
    ...
    pConf->Stats->OscTolC1     = (((pConf->NSJW + 1) * 10000) / (2 * 10 *
                                                                 NTQbits));                                                          // Condition 1 for the maximum tolerance of the oscillator with 2 digits after the decimal point (Equation 3-12 of MCP25XXFD Family Reference Manual)
    pConf->Stats->OscTolerance = pConf->Stats->OscTolC1;
    pConf->Stats->OscTolC2     = ((MinNPHSEG * 10000) / (2 * (13 * NTQbits - (pConf->NTSEG2 +
                                                                              1))));                                          // Condition 2 for the maximum tolerance of the oscillator with 2 digits after the decimal point (Equation 3-13 of MCP25XXFD Family Reference Manual)
    pConf->Stats->OscTolerance = (pConf->Stats->OscTolC2 < pConf->Stats->OscTolerance ? pConf->Stats->OscTolC2 :
                                  pConf->Stats->OscTolerance); // Oscillator Tolerance, minimum of conditions 1-5 (Equation 3-11 of MCP25XXFD Family Reference Manual)
    if (can20only) {
        pConf->Stats->OscTolC3   = 0;
        pConf->Stats->OscTolC4   = 0;
        pConf->Stats->OscTolC5   = 0;
    } else {
        pConf->Stats->OscTolC3   = (((pConf->DSJW + 1) * 10000) / (2 * 10 *
                                                                   DTQbits));                                                                                   // Condition 3 for the maximum tolerance of the oscillator with 2 digits after the decimal point (Equation 3-14 of MCP25XXFD Family Reference Manual)
        pConf->Stats->OscTolerance = (pConf->Stats->OscTolC3 < pConf->Stats->OscTolerance ? pConf->Stats->OscTolC3 :
                                      pConf->Stats->OscTolerance);                        // Oscillator Tolerance, minimum of conditions 1-5 (Equation 3-11 of MCP25XXFD Family Reference Manual)
        const uint32_t NBRP = (pConf->NBRP + 1), DBRP = (pConf->DBRP + 1);
        pConf->Stats->OscTolC4   = ((MinNPHSEG * 10000) / (2 * ((((6 * DTQbits - (pConf->DTSEG2 + 1)) * DBRP) / NBRP) + (7 *
                                                                NTQbits))));                                // Condition 4 for the maximum tolerance of the oscillator with 2 digits after the decimal point (Equation 3-15 of MCP25XXFD Family Reference Manual)
        pConf->Stats->OscTolerance = (pConf->Stats->OscTolC4 < pConf->Stats->OscTolerance ? pConf->Stats->OscTolC4 :
                                      pConf->Stats->OscTolerance);                        // Oscillator Tolerance, minimum of conditions 1-5 (Equation 3-11 of MCP25XXFD Family Reference Manual)
        const int32_t NBRP_DBRP = ((NBRP * 10000) / DBRP), MaxBRP = ((NBRP_DBRP - 10000) > 0 ? (NBRP_DBRP - 10000) :
                                                                     0);                                                 // NBRP/DBRP and max(0,(NBRP/DBRP-1)). The use of 10000 is to set 2 digits on the C5 result
        pConf->Stats->OscTolC5   = ((((pConf->DSJW + 1) * 10000) - MaxBRP) / (2 * (((2 * NTQbits - (pConf->NTSEG2 + 1)) * NBRP) / DBRP + (pConf->DTSEG2 + 1) + 4 *
                                                                                   DTQbits))); // Condition 5 for the maximum tolerance of the oscillator with 2 digits after the decimal point (Equation 3-16 of MCP25XXFD Family Reference Manual) [WARNING: An error seems to be present in the original formula]
        pConf->Stats->OscTolerance = (pConf->Stats->OscTolC5 < pConf->Stats->OscTolerance ? pConf->Stats->OscTolC5 :
                                      pConf->Stats->OscTolerance);                        // Oscillator Tolerance, minimum of conditions 1-5 (Equation 3-11 of MCP25XXFD Family Reference Manual)
    }
    return ERR_NONE;
}
...
eERRORRESULT MCP251XFD_RequestOperationMode(MCP251XFD *pComp, eMCP251XFD_OperationMode newMode, bool waitOperationChange)
{
    ...
    pComp->InternalConfig |= MCP251XFD_DEV_PS_SET(
                                 MCP251XFD_DEVICE_NORMAL_POWER_STATE); // Set normal power state even if the operation mode is sleep, this value will be changed to the good value by the function MCP251XFD_EnterSleepMode()
    return ERR_NONE;
}
...
eERRORRESULT MCP251XFD_WaitOperationModeChange(MCP251XFD *pComp, eMCP251XFD_OperationMode askedMode)
{
    ...
    while (true) {
        ...
        if (MCP251XFD_TIME_DIFF(StartTime, pComp->fnGetCurrentms()) >
            7) { // Wait at least 7ms because the longest message is 731 bit long and the minimum bitrate is 125kbit/s that mean 5,8ms + 2x 6bytes @ 1Mbit/s over SPI that mean 96µs = ~6ms + 1ms because GetCurrentms can be 1 cycle before the new ms
...
eERRORRESULT MCP251XFD_WakeUp(MCP251XFD *pComp, eMCP251XFD_PowerStates *fromState)
{
    ...
    Error = MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_OSC_CONFIG,
                               &Config);                // Read the Oscillator Register configuration (Here if the device is in DEVICE_LOWPOWER_SLEEP_STATE, the reset already happened because a simple assert of SPI CS exit the LPM)
    if (Error != ERR_NONE) { return Error; }                                                // If there is an error while reading the SFR then return the error
...
{
    // Validate prescaler as user-facing value in range [1 .. MAX+1]
    if (prescaler < (MCP251XFD_CAN_CiTSCON_TBCPRE_MINVALUE + 1u)) { return ERR__PARAMETER_ERROR; }
    if (prescaler > (MCP251XFD_CAN_CiTSCON_TBCPRE_MAXVALUE + 1u)) { return ERR__PARAMETER_ERROR; }

    eERRORRESULT Error;
    MCP251XFD_CiTSCON_Register Reg;   // Raw register view

    // Build register (write only lower 3 bytes: TBCPRE + control bits up to bit 23)
    Reg.CiTSCON = MCP251XFD_CAN_CiTSCON_TBCDIS;  // default: TBC disabled, SOF anchor, FD@SOF

    if (enableTS) {
        // Enable TBC
        Reg.CiTSCON |= MCP251XFD_CAN_CiTSCON_TBCEN;
        // Apply anchor/resolution combination derived from enum (EOF / FD-FDF)
        Reg.CiTSCON |= MCP251XFD_CAN_CiTSCON_MODE_FROM_ENUM(samplePoint);
        // Program prescaler as (prescaler - 1)
        Reg.CiTSCON |= MCP251XFD_CAN_CiTSCON_TBCPRE_SET((uint32_t)(prescaler - 1u));
    }

    // Write first 3 bytes of CiTSCON (bits [23:0])
    Error = MCP251XFD_WriteData(pComp, RegMCP251XFD_CiTSCON, &Reg.Bytes[0], 3u);
    if (Error != ERR_NONE) { return Error; }

    // Configure CiINT.TBCIE according to interruptBaseCounter
    uint8_t intCfg;
    Error = MCP251XFD_ReadSFR8(pComp, RegMCP251XFD_CiINT_CONFIG, &intCfg);  // read the third byte view
    if (Error != ERR_NONE) { return Error; }

    if (interruptBaseCounter) { intCfg |=  MCP251XFD_CAN_CiINT8_TBCIE; }    // enable TBC interrupt
    else                      { intCfg &= ~MCP251XFD_CAN_CiINT8_TBCIE; }    // disable TBC interrupt

    return MCP251XFD_WriteSFR8(pComp, RegMCP251XFD_CiINT_CONFIG, intCfg);
}
